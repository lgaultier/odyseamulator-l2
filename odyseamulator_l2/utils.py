...
def computeEncoderByXT(cross_track):
    ...
    Returns encoder_angle_fore, encoder_angle aft, the forward and backward
    looking samples. Degrees clockwise from the velocity vector.
    """
...
def getBearing(platform_latitude, platform_longitude):
    ...
    lon_diff = signedAngleDiff(platform_longitude[0:-d]*180/np.pi,platform_longitude[d::]*180/np.pi)*np.pi/180
    X[d::] = np.cos(platform_latitude[d::]) * np.sin(lon_diff)
    Y[d::] = (np.cos(platform_latitude[0:-d]) * np.sin(platform_latitude[d::])
              - np.sin(platform_latitude[0:-d])
              * np.cos(platform_latitude[d::]) * np.cos(lon_diff))
...
def SDToUVErrors(magnitude, direction, magnitude_error, direction_error):
    ...
    u_error = np.abs(sin_dir * magnitude) * np.sqrt((std_sin_dir / sin_dir)**2
                                              + (magnitude_error/magnitude)**2)
    v_error = np.abs(cos_dir * magnitude) * np.sqrt((std_cos_dir / cos_dir)**2
                                              + (magnitude_error/magnitude)**2)
